...
game_sim_data <- savant_data_filtered %>%
  mutate(pitch_type = case_when(pitch_type == "KC" ~ "CU",
                                TRUE ~ pitch_type)) %>%
  filter(pitch_type %in% c("FF", "CH", "SL", "FT", "SI", "CU", "FC")) %>%
  select(player_name, pitch_type, events, stand, p_throws, strikes, balls, 
         outs_when_up, on_1b, on_2b, on_3b, hc_x, hc_y)
...
save(game_sim_data, file = "data/game_sim.Rdata")
